...
nytw$year <- as.numeric(gsub("https?://(?:www.)?nytimes.com/([0-9]+)/.*", "\\1", nytw$url, fixed=F, perl=T))

nytw$valid <- complete.cases(nytw) &
    nytw$bride_age > 0 & nytw$groom_age > 0 &
    nytw$year == 2014 & nytw$name_status %in% c('taking', 'keeping')
nytwv <- nytw[nytw$valid == T,] #put it in a new DF
...
fit <- svm(name_status ~ groom_age + bride_age, data=nytwv, kernel='linear', cost=1/500, scale=F)
